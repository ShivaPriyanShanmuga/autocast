...
const x264Check = ffmpegLibCheck('libx264', (name) => ({
  ...
  hint: '    castscript encodes H.264 and this ffmpeg cannot. Install a full build:\n' +
    FFMPEG_INSTALL,
}));
      ...
      return {
        ...
        hint: [
          ...
          ...(error instanceof Error ? error.message : String(error))
            ...
            .map((line) => `    ${line}`),
          '',
          '    castscript drives a real PTY for terminal scenes.',
  ...
  async run() {
    ...
    try {
      ...
      return {
        ...
        hint: [
          '    Voice is opt-in: castscript does not install a speech engine by',
