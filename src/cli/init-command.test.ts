...
function scratch(files: Record<string, string> = {}): string {
  const dir = mkdtempSync(join(tmpdir(), 'autocast-init-'));
  dirs.push(dir);
...
describe('initCommand', () => {
  it('writes a demo script and the schema', async () => {
    ...
    expect(existsSync(join(dir, '.autocast', 'schema.json'))).toBe(true);
  });
  ...
  it('writes exactly the schema `autocast schema` prints', async () => {
    ...
    expect(readFileSync(join(dir, '.autocast', 'schema.json'), 'utf8').trim()).toBe(
      s.out.join('').trim(),
    );
  });

  it('refuses to clobber an existing demo script', async () => {
    const dir = scratch({ 'demo.yaml': 'autocast: 1 # hand written\n' });
    const { io: cio, err } = io();
  ...
  it('tells the agent what to run next', async () => {
    ...
    await initCommand(['--dir', dir], cio);
    const text = out.join('');
    expect(text).toMatch(/autocast validate/);
    expect(text).toMatch(/autocast render/);
  });
});
