# autocast Phase 2a — Browser capture — Implementation Plan
...
**Spec:** `docs/superpowers/specs/2026-09-04-autocast-design.md` — especially §4.1, §4.5, §4.5.1, §4.6 and §8.
...
<html lang="en">
  <head>
    <meta charset="utf-8" />
    <title>autocast order desk</title>
    <style>
  ...
  <body>
    <header><h1>autocast order desk</h1></header>
    <main>
      <div class="card">
...
A dependency-free HTTP server used by autocast's browser tests. Never add
...
async function session() {
  const dir = mkdtempSync(join(tmpdir(), 'autocast-ba-'));
  dirs.push(dir);
...
- Produces:
  ...
  - New option: `framesRoot?: string` — where browser frames are written (default `.autocast/frames`)
...
- `autocast doctor` reports a `chromium` row.
- `autocast validate fixtures/browser/demo.yaml` exits 0.
