# castscript Phase 7 — Agent surface and release — Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** An agent in a repo that has never seen castscript writes a demo script and gets an mp4, without a human explaining the schema.

**Architecture:** No new runtime machinery. §9 already settled that the product is `npx castscript` plus knowledge, not an MCP server: the agent's whole job is write-a-file, run-a-command, read-a-report, which every coding agent can already do. This phase adds the missing knowledge layer and the one missing command.

**Tech Stack:** No new dependencies.

**Spec:** `docs/superpowers/specs/2026-09-04-castscript-design.md` — especially §9 and §10.

## Decisions taken into this plan

- **Not publishing to npm in this phase.** `castscript` is taken (v0.0.4 by someone else). Everything ships and is verified locally; publishing stays a deliberate step behind a name the author picks.
- **The skill must NOT embed the schema.** §9 rejected MCP partly for adding "a duplicated schema to keep in sync"; copying the schema into a skill file would be that same mistake in a cheaper wrapper. The skill tells the agent to run `castscript schema`.
...
- **Knowledge lives in one place.** Schema from `castscript schema`; the skill and `AGENTS.md` point at it rather than restating it.
...
Writes `demo.yaml` scaffolded for what it found, and `.castscript/schema.json`
...
- [x] **Step 1: Failing tests** — writes `demo.yaml` and
  `.castscript/schema.json`; the scaffold passes `castscript validate`
  ...
  matches `castscript schema` byte for byte.
- [x] **Step 2: Implement**
...
- [x] **Step 1: Write it**, pointing at `castscript schema` rather than
...
**Files:**
- Create: `skills/castscript/SKILL.md`

Thin by design: frontmatter, when to use it, the loop, and a pointer to
`castscript schema`. Everything the skill would otherwise duplicate is a
command away.
...
temporary directory that has never seen castscript, run `init`, then
...
1. In a fresh directory, `npx castscript init && npx castscript render demo.yaml`
   produces a playable mp4 with no hand-holding.
2. `castscript init` refuses to clobber an existing `demo.yaml`.
