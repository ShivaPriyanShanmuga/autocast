# castscript Phase 4 — Verification hardening — Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Make it structurally impossible for castscript to hand back a plausible-looking video of a broken demo.
...
**Spec:** `docs/superpowers/specs/2026-09-04-castscript-design.md` — especially §3 constraint 2, §8, §10 and §12.
...
**Interfaces:**
- Produces:
  - `interface ReportCore { castscript: 1; ok: boolean; scenes: Array<{ id: string; ok: boolean; assertions: Array<{ name: string; ok: boolean; detail?: string }> }>; findings: Finding[]; abortedAt: string | null }`
...
const dir = mkdtempSync(join(tmpdir(), 'castscript-report-'));
...
const dir = mkdtempSync(join(tmpdir(), 'castscript-sheet-'));
afterAll(() => rmSync(dir, { recursive: true, force: true, maxRetries: 5, retryDelay: 100 }));
...
scene, write `.castscript/failed/<scene>-contact.png`, and put the path on
