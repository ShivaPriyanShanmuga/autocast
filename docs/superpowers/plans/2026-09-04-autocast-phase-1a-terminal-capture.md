# autocast Phase 1a — Terminal capture — Implementation Plan
...
**Spec:** `docs/superpowers/specs/2026-09-04-autocast-design.md` — especially §4.1, §4.2, §4.6, §6 and §6.1.
...
export async function executeStep(step: AnyStep, ctx: StepContext): Promise<StepResult> {
  ...
  if (typeof step.run === 'string') {
    const marker = `__autocast_done_${Math.random().toString(36).slice(2, 8)}__`;
...
scenes:
  - id: greet
    ...
    steps:
      - type: "node fixtures/cli-app/cli.mjs greet autocast"
      - key: Enter
      - wait_for:
          stdout: /ready/
    assert:
      - stdout_contains: "hello autocast"
...
describe('captureTerminalDemo', () => {
  ...
  it('produces one asciicast per session, containing real output', async () => {
    ...
    expect(cast!.events.length).toBeGreaterThan(0);
    expect(cast!.events.map((e) => e[2]).join('')).toContain('hello autocast');
  }, 90000);
...
- `captureTerminalDemo` drives `fixtures/terminal/demo.yaml` end to end: two scenes, one shared session, all assertions green, an asciicast containing `hello autocast`.
