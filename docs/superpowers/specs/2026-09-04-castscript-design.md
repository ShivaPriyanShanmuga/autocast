# castscript — agent-driven demo video recorder
...
- **`narrate` exists from day one even while silent.** Phase 1 uses it for
  ...
  schema change.
- **`castscript: 1`** is a schema version supporting migration.
...
`castscript doctor` reports it as `skip` rather than `fail` when absent, and a
...
- **The default model cache is unloadable on Windows.** transformers.js caches
  ...
  onnxruntime then reported that a valid 92MB file did not exist. castscript
...
**Layer 1 — static (`castscript validate`), zero execution.** Schema check;
...
```
castscript render - docs/demo.mp4
  ...
  Contact sheet: .castscript/failed/logs-contact.png   (not read automatically)
```
...
**Decision:** `npx castscript` is the product. A thin Claude Code skill packages
...
```
castscript init        -> scaffold + schema reference
  (agent writes demo.yaml)
castscript validate    -> static errors, no execution, ~free
  (agent fixes)
castscript render      -> text report (~300 tokens)
```
...
*Exit:* `npx castscript doctor` truthfully reports present/missing deps with
copy-pasteable install commands. `castscript validate fixtures/mixed/demo.yaml`
...
`castscript init`, and a failure playbook that turns each failure into one
...
**Publishing is deliberately NOT part of this phase.** `castscript` is taken on
