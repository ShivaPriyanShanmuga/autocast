...
# Recording a demo video with autocast
...
```
npx autocast init                  # scaffold demo.yaml + local schema
npx autocast validate demo.yaml    # static check, no capture — free
npx autocast render demo.yaml      # records, encodes, prints a report
```
...
Run `npx autocast schema`. Do not reconstruct it from memory or from an
example — this file deliberately does not restate it, so the two cannot
drift apart.

`init` also writes `.autocast/schema.json` and references it from the
...
`AGENTS.md` in the autocast repo carries the failure table and the
